...
import { ErrorCard, LoadingCard, useFilteredDocs, useMissionControlData } from "@/components/mission-control/data-loader";
...
export default function MissionControlDocsPage() {
  ...
  const filteredDocs = useFilteredDocs(data, docQuery);

  useEffect(() => {
    if (data?.docs[0] && !selectedDoc) setSelectedDoc(data.docs[0].name);
  }, [data, selectedDoc]);
